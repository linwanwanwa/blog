    ...
    <div id="gameContainer">
        ...
        <div id="gameMenu" v-if="gameState === 'menu'">
            ...
            <div class="menu-section">
                ...
                <div class="mode-buttons">
                    ...
                    <button 
                        class="mode-btn" 
                        :class="{ active: selectedMode === 'double' }"
                        @click="selectedMode = 'double'"
                    >
                        <span class="mode-icon">👥</span>
        ...
        <div id="gameOver">
            ...
            <div class="game-over-stats">
                ...
                <div class="stat-item" v-if="mode === 'single'">
                    <span class="stat-label">最终得分</span>
                    <span class="stat-value" id="finalSingleScore">0</span>
                </div>
                <div class="stat-item" v-if="mode === 'double'">
                    <span class="stat-label">P1 得分</span>
                    <span class="stat-value" id="finalP1Score">0</span>
                </div>
                <div class="stat-item" v-if="mode === 'double'">
                    <span class="stat-label">P2 得分</span>
                    <span class="stat-value" id="finalP2Score">0</span>
                </div>
            </div>
...
const difficulties = [
    { id: 'easy', name: '简单', desc: '鱼少且慢', speedMult: 0.6, spawnRate: 0.7 },
    { id: 'normal', name: '普通', desc: '平衡体验', speedMult: 1, spawnRate: 1 },
    { id: 'hard', name: '困难', desc: '鱼多且快', speedMult: 1.4, spawnRate: 1.3 },
    { id: 'nightmare', name: '噩梦', desc: '极限挑战', speedMult: 1.8, spawnRate: 1.6 }
]
...
class Fish {
    constructor(type = 'normal') {
        ...
        const speedMult = game.speedMult
...
function spawnFish() {
    ...
    const spawnRate = game.spawnRate || 1
...
function updateUI() {
    ...
    if (mode.value === 'double') {
        ...
        document.getElementById('speedDisplay').textContent = game.speedMult.toFixed(1) + 'x'
    }
}
...
function checkLevelUp() {
    if (mode.value === 'single') {
        if (player1.score >= 100 * game.level) {
            game.level++
            game.spawnInterval = Math.min(game.spawnInterval + 15 / (game.spawnRate || 1), 80)
...
function update() {
    ...
    const spawnInterval = game.spawnInterval * (game.spawnRate || 1)
...
.game-title {
    font-size: 42px;
    margin-bottom: 8px;
    background: linear-gradient(135deg, #4ecdc4, #44a08d, #ff6b6b);
